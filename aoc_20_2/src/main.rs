...
fn main() {
    ...
    input.iter()
        ...
        .for_each(|item| list.push_back(item*DECRYPTION_KEY));
    ...
    // Apply the algorithm for part 1 ten times
    list.origin_iter_mut().cycle().take((list_length*10).try_into().unwrap())
    .for_each(|item| {
        let value = *item.get();
